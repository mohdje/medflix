...
namespace MoviesAPI.Services.Torrent
{
    internal abstract class TorrentVFSearcher : ITorrentVFMovieSearcher, ITorrentSerieSearcher
    {
        ...
        public async Task<IEnumerable<MediaTorrent>> GetTorrentLinksAsync(string originalMovieName, string frenchMovieName, int year)
        {
            var request = new TorrentVFMovieSearchRequest(originalMovieName, frenchMovieName, year);
        ...
        protected async Task<string[]> GetTorrentLinkAsync(string moviePageUrl)
        {
            ...
            var magnetNode = doc.DocumentNode.SelectSingleNode(MagnetButtonIdentifier);
            var directDownloadNode = doc.DocumentNode.SelectSingleNode(TorrentButtonIdentifier);
